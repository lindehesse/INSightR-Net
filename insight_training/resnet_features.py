...
def resnet18_features(pre_imagenet=False, pre_path = None,  **kwargs):
    ...
    if pre_path:
        ...
        for key in my_dict.keys():
            if key[:6] == 'model.':
                new_statedict[key[6:]] = my_dict[key]
        new_statedict.pop('fc.weight')
        new_statedict.pop('fc.bias')
        model.load_state_dict(new_statedict, strict=True)
    return model
...
if __name__ == '__main__':
    ...
    r18_features = resnet18_features(pretrained=True, pretrained_DR=True)
    print(r18_features)
